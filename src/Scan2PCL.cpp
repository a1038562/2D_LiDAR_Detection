...
Scan2PCL::Scan2PCL(): max_range(5.0), min_range(0.01)
{
    ...
    // pub_all = nh.advertise<sensor_msgs::PointCloud2>("/pcl2_all", 1); //PCL2 ALL Angle Publish
}
...
void Scan2PCL::Callback(const sensor_msgs::LaserScan laser_msg)
{
    ...
    for(size_t i=0; i<range_size; i++)
    {
        ...
        { 
            ...
            if (angle >= 90 && angle <= 270.0) // ROI angle
            {
    ...
    // for(size_t i=0; i<range_size; i++)
    // {
    //     if(laser_ranges[i] > min_range && laser_ranges[i] < 5.0) // ROI range
    //     { 
    //         angle = laser_msg.angle_min + 360.0 * (float(i) / float(range_size));
    //         x = (1) * std::get<0>(coordinate);
    //         y = (-1) * std::get<1>(coordinate);
